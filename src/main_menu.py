...
def key_pressed_event(event):
    ...
    if button_bind_selected is not None:
        ...
        if key_code == 27:
            button_bind_selected.configure(text=button_bind_selected.cget('text')[1:-1].capitalize(),
                                           text_color="black")
            button_bind_selected = None
            return

        for bind in bind_options:
            if bind[1][0] == key_code and bind[0] != bind_options[button_bind_selected.bind_id][
                0]:  # We don't care about assigning same value on the same bind
                # match, can't assign this twice
                tkinter.messagebox.showwarning(title="Conflit",
                                               message=f"Cette touche a déjà été assigné à '{bind[0]}' !")
                button_bind_selected.configure(text=button_bind_selected.cget('text')[1:-1].capitalize(),
                                               text_color="black")
                button_bind_selected = None
                return
...
def run(application: tkinter.Tk, username: str):
    ...
    button_play = customtkinter.CTkButton(master=frame_main_menu, text="Jouer", corner_radius=0, height=65,
                                          fg_color=button_color, hover_color=button_color_hover,
                                          font=("Arial Bold", 20), text_color="white", width=width_button,
                                          cursor="hand2", command=play_button_pressed)
    button_play.grid(row=0, column=0, sticky="EW", pady=(10, 5), padx=padx_button)

    button_leaderboard = customtkinter.CTkButton(master=frame_main_menu, text="Leaderboard", corner_radius=0, height=65,
                                                 fg_color=button_color, hover_color=button_color_hover,
                                                 font=("Arial Bold", 20), text_color="white", width=width_button,
                                                 cursor="hand2", command=show_leaderboard)
    button_leaderboard.grid(row=1, column=0, sticky="EW", pady=5, padx=padx_button)

    button_option = customtkinter.CTkButton(master=frame_main_menu, text="Options", corner_radius=0, height=65,
                                            fg_color=button_color, hover_color=button_color_hover,
                                            font=("Arial Bold", 20), text_color="white", width=width_button,
                                            cursor="hand2", command=show_options)
    button_option.grid(row=2, column=0, sticky="EW", pady=5, padx=padx_button)

    button_quit = customtkinter.CTkButton(master=frame_main_menu, text="Quitter", corner_radius=0, height=65,
                                          fg_color=button_color, hover_color=button_color_hover,
                                          font=("Arial Bold", 20), text_color="white", width=width_button,
                                          cursor="hand2", command=lambda: app.quit())
    button_quit.grid(row=3, column=0, sticky="EW", pady=(5, 10), padx=padx_button)

    # leaderboard menu
    frame_leaderboard = customtkinter.CTkFrame(master=image_canvas, corner_radius=0, fg_color=frame_color,
    ...
    label_leaderboard = customtkinter.CTkLabel(master=frame_leaderboard, width=300, corner_radius=0,
                                               fg_color=frame_color, bg_color=frame_color, text="Leaderboard",
                                               font=(0, 30), text_color="White")
    ...
    scrollable_frame = customtkinter.CTkScrollableFrame(width=250, master=frame_leaderboard,
                                                        fg_color=scrollable_color_bg, border_color=frame_color)
    ...
    for i in range(len(leaderboard_data)):
        username_label = customtkinter.CTkLabel(master=scrollable_frame, corner_radius=0, text=leaderboard_data[i][0],
                                                text_color="White", font=(0, 14))
        score_label = customtkinter.CTkLabel(master=scrollable_frame, corner_radius=0, text=leaderboard_data[i][1],
                                             text_color="White", font=(0, 14))
    ...
    button_back = customtkinter.CTkButton(master=frame_leaderboard, text="Menu principal", corner_radius=0, height=50,
                                          fg_color=button_color, hover_color=button_color_hover,
                                          font=("Arial Bold", 20), text_color="white", cursor="hand2",
                                          command=lambda: show_main_menu("leaderboard"))
    button_back.grid(row=2, column=0, sticky="EW", padx=15, pady=(0, 10))

    # options menu

    frame_options = customtkinter.CTkFrame(master=image_canvas, corner_radius=0, fg_color=frame_color,
                                           bg_color=frame_color)
    ...
    label_options = customtkinter.CTkLabel(master=frame_options, width=300, corner_radius=0, fg_color=frame_color,
                                           bg_color=frame_color, text="Options", font=(0, 30), text_color="White")
    label_options.grid(row=0, column=0, sticky="S", pady=10)

    scrollable_options = customtkinter.CTkScrollableFrame(width=250, master=frame_options, fg_color=scrollable_color_bg,
                                                          border_color=frame_color)
    scrollable_options.grid(row=1, column=0, pady=10)

    label_slider = customtkinter.CTkLabel(master=scrollable_options, corner_radius=0, text="Volume", font=(0, 14),
                                          text_color="White")
    ...
    for i in range(len(bind_options)):
        data = bind_options[i]
        label_option_bind = customtkinter.CTkLabel(master=scrollable_options, corner_radius=0, text=data[0],
                                                   font=(0, 14), text_color="White")
        button_bind = customtkinter.CTkButton(master=scrollable_options, text=data[1][1], corner_radius=0, height=30,
                                              fg_color=button_color, hover_color=button_color_hover,
                                              font=("Arial Bold", 13), text_color="black", cursor="hand2")
    ...
    button_back = customtkinter.CTkButton(master=frame_options, text="Menu principal", corner_radius=0, height=50,
                                          fg_color=button_color, hover_color=button_color_hover,
                                          font=("Arial Bold", 20), text_color="white", cursor="hand2",
                                          command=lambda: show_main_menu("options"))
    button_back.grid(row=2, column=0, sticky="EW", padx=15, pady=(0, 10))
