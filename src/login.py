...
def perform_login():
    ...
    elif not utils.username_exist(username_entry.get()) or not utils.check_password(username_entry.get(),
                                                                                    password_entry.get()):
        tkinter.messagebox.showwarning(title="Erreur",
...
def run():
    ...
    frame_components = customtkinter.CTkFrame(login_canvas, fg_color="white", bg_color="white",
                                              width=login_canvas.winfo_width() - 30)
    ...
    username_entry = customtkinter.CTkEntry(master=frame_components, placeholder_text="Pseudo", height=70,
                                            fg_color="#D9D9D9", corner_radius=0, border_color="#D9D9D9",
                                            bg_color="#D9D9D9", text_color="#9A9A9A", font=("", 20))
    username_entry.image = utils.get_image("user_icon.png", 64, 64)
    username_entry.grid(row=0, column=0, sticky="sew", padx=(25, 0), pady=5)

    image_username_label = customtkinter.CTkLabel(frame_components, text="", width=0, height=70,
                                                  image=username_entry.image, fg_color="#D9D9D9", bg_color="#D9D9D9")
    image_username_label.grid(row=0, column=1, sticky="se", padx=(0, 25), pady=5)

    global email_entry, email_entry_label

    email_entry = customtkinter.CTkEntry(master=frame_components, placeholder_text="Email", height=70,
                                         fg_color="#D9D9D9", corner_radius=0, border_color="#D9D9D9",
                                         bg_color="#D9D9D9", text_color="#9A9A9A", font=("", 20))
    email_entry.image = utils.get_image("email_icon.png", 64, 64)

    email_entry_label = customtkinter.CTkLabel(frame_components, text="", width=0, height=70, image=email_entry.image,
                                               fg_color="#D9D9D9", bg_color="#D9D9D9")

    password_entry = customtkinter.CTkEntry(master=frame_components, placeholder_text="Mot de passe", height=70,
                                            fg_color="#D9D9D9", corner_radius=0, border_color="#D9D9D9",
                                            bg_color="#D9D9D9", text_color="#9A9A9A", show='*', font=("", 20))
    password_entry.view_password = False
    ...
    image_password_label = customtkinter.CTkLabel(frame_components, text="", height=70, image=password_entry.image,
                                                  fg_color="#D9D9D9", bg_color="#D9D9D9")
    ...
    frame_components_bottom = customtkinter.CTkFrame(login_canvas, fg_color="white", bg_color="white", height=90,
                                                     width=login_canvas.winfo_width() - 30)
    ...
    label_switch_window = customtkinter.CTkLabel(master=frame_components_bottom, text="Créer un compte",
                                                 text_color="#9A9A9A", fg_color="white", bg_color="white",
                                                 font=("Arial Bold", 20))
    ...
    button_perform = customtkinter.CTkButton(master=frame_components_bottom, text="Se connecter", corner_radius=0,
                                             height=65, fg_color="#67E9DA", hover_color="#436e77",
                                             font=("Arial Bold", 20), text_color="white", width=200, cursor="hand2",
                                             command=perform_button)
